...
class NewOnlineAdaboost():
    ...
    def fit(self, X_train, Y_train):
        ...
        for X, y in tqdm(zip(X_train, Y_train), total=X_train.shape[0]):   
            ...
            #     print(f">> {index}: epsilon {self.epsilon[index]} - v {self.v[index]} - P {P_iter[index]}")
            ...
            self.alphas = np.maximum(self.beta * np.log(np.maximum((1 - self.epsilon) / (np.maximum(self.epsilon, self.eta)), self.eta)) \
