...
def plot_one_data_multi_norm(X, y, means, stds): 
    ...
    nrows = int(np.ceil(hyper.n_features / 2.0))
    fig, axs = plt.subplots(nrows=nrows, ncols=2) 
    if axs.shape != (nrows, 2):
        axs = axs[None, ]
    for row in range(nrows): 
        for col in range(2):
            if 2*row + col >= hyper.n_features: break; 
            findex = 2*row + col
            ...
            if X is not None:
                if y == 1:
                    axs[row, col].plot(X[findex], 0, c='b', marker='o')
                else:
                    axs[row, col].plot(X[findex], 0, c='r', marker='o')
...
def plot_multi_norm_data(X, y, means, stds, label_indexs, label_names, feature_names): 
    """Plot multiple normal distribution graph
    Args:
        X (_array (N_samples, N_features)_): dataset
        y (_N_samples_): labels
